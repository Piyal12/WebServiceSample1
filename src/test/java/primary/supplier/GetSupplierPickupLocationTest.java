...
public class GetSupplierPickupLocationTest extends TestBase {
	...
	@Test (enabled=true, dataProviderClass=TestDataProvider.class, dataProvider="TestDataGenerator",groups = {"regression","sanity","positive"},description = "GetSupplierPickupLocationTest with all valid params")
	...
	public void getSupplierLocationTest(HashMap <String, String> data) {
		WebService rest = Ws.get(ServiceGroup.SUPPLIER,GetSupplierPickupLocationTest.SERVICE_NAME,TestBase.ENV,TOKEN,data);
		LOG.info("Status:" + rest.getStatus());
		Assert.assertEquals(rest.getStatus(), 200, "The expected status is "+200+". But actual is "+rest.getStatus()+".");
		Assert.assertTrue(rest.getResponse().body().jsonPath().getBoolean("success") , "getSupplierLocation failed");
